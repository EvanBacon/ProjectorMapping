...
static void depthCallback(freenect_device *dev, void *depth, uint32_t timestamp) {
	[(__bridge Kinect_Controller*)freenect_get_user(dev) depthCallback:(uint16_t*)depth];
}
static void rgbCallback(freenect_device *dev, void *video, uint32_t timestamp) {
    [(__bridge Kinect_Controller*)freenect_get_user(dev) rgbCallback:(uint8_t*)video];
}
static void irCallback(freenect_device * dev, void *video, uint32_t timestamp) {
    [(__bridge Kinect_Controller*)freenect_get_user(dev) irCallback:(uint8_t*)video];
}
...
- (void)kinectThread {
	...
	if (freenect_init(&context, NULL) >= 0) {
		if (freenect_num_devices(context) == 0) {
			...
			freenect_set_user(_kinectDevice, (__bridge void *)(self));
